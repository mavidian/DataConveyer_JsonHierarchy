...
namespace DataConveyer_JsonHierarchy
{
   class Program
   {
      ...
      static void Main()
      {
         ...
         Console.WriteLine($"DataConveyer library used: {ProductInfo.CurrentInfo}");
         Console.WriteLine();
         Console.WriteLine("This application transforms nesting hierarchy of JSON data.");
         Console.WriteLine();
      ...
      private static async void FileDroppedHandler(object sender, FileSystemEventArgs e)
      {
         ...
         {
            Console.WriteLine($"Processing {fname} file completed in {stopWatch.Elapsed.TotalSeconds:##0.000}s; {result.ClustersWritten:N0} out of {result.ClustersRead:N0} clusters posted onto output.");
         }
